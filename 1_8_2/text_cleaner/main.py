...
class TextCleaner(Skill):
    ...
    async def validate(self) -> list[WingmanInitializationError]:
        ...
        self.remove_thinking = self.retrieve_custom_property_value("remove_thinking", errors)
        return errors
    ...
    async def on_play_to_user(self, text: str, sound_config: dict) -> str:
        ...
        # Helper function to remove portions surround by thinking indicator
        def remove_thinking_phrases(input_text):
            #regex_pattern = re.compile(r"<think>(.*?)</think>", re.DOTALL)

            #"<think>(\S+\s+\S+.*?)</think>" #r"<think>(.*?)</think>"
            regex_pattern = "<think>(\S+\s+\S+.*?)</think>"
            # Find the phrases in the text, using re.DOTALL to match across newlines
            thinking_phrases = re.findall(
                regex_pattern, 
                self.message_to_clean, 
            )
            # Remove them from the markdown-free text passed in the text variable of on_play_to_user
            thinking_removed_text = input_text
            for phrase in thinking_phrases:
                thinking_removed_text = thinking_removed_text.replace(phrase, "")
            return thinking_removed_text
        ...
        # Remove thinking phrases (portions surrounded by <think> and </think> tokens)
        if self.remove_thinking:
            cleaned_text = remove_thinking_phrases(cleaned_text)
