...
if TYPE_CHECKING:
    from wingmen.wingman import OpenAiWingman
...
class XPlane(Skill):
    ...
    async def _read_aircraft_and_environment_data(self, variable: str) -> str:
        ...
        dataref = b'sim/aircraft/engine/acf_num_engines'
        msg = struct.pack("<4sxii400s", b'RREF',
                        1,                     # Send data # times/second
                        0,                    # include this index number with results
                        dataref)  # remember to encode as bytestring
        # add timeout to socket
        self.sock.settimeout(10.0)
        try:
            self.sock.sendto(msg, (self.x_plane_ip, self.x_plane_port))
        except Exception as e:
            print(f"Error transmitting data ref to xplane, {e}.")

        try:
            data, addr = self.sock.recvfrom(2048)
        except Exception as e:
            print(f"Encountered error with recieving data in socket, {e}.")
        header = data[0:4]
